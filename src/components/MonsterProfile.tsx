...
import { Pill, Spinner, ErrorState, SectionHeader } from "./ui";
import DofusIcon, { type DofusIconName } from "./DofusIcon";
import { SpellRangeMap, SpellDetail, mapSubtitle, spellLevelForGrade } from "./SpellRangeMap";
...
// Profil monstre complet (stats, sorts, zone ciblable, butin, donjons, famille).
// Extrait de la page Monstres pour être réutilisé dans les avis de recherche.
// `embedded` (avis de recherche) masque le portrait + nom, déjà présents dans l'en-tête de l'avis.
export default function MonsterProfile({ monsterId, embedded = false }: { monsterId: number; embedded?: boolean }) {
  ...
  const { data: family } = useQuery({
    ...
    enabled: monster?.race != null && !embedded,
  });
  ...
  return (
    <div>
      {/* En-tête : portrait + identité + sélecteur de niveau + stats du grade.
          En mode embarqué (avis de recherche) on masque portrait + nom : déjà dans l'en-tête de l'avis. */}
      <div className="glass relative overflow-hidden rounded-3xl p-5">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_30%,rgba(244,63,94,0.12),transparent_45%)]" />
        {embedded ? (
          <div className="relative space-y-4">
            {(raceName || monster.isBoss || grades.length > 1) && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <GradeSelector grades={grades} gradeIdx={gradeIdx} onSelect={setGradeIdx} />
              </div>
            )}
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="font-display text-3xl font-extrabold leading-tight text-white">{monster.name.fr}</h2>
      ...
      {/* Même famille (race) — clic = navigation directe vers le monstre.
          Masquée en mode embarqué (avis de recherche). */}
      {!embedded && family && family.filter((m) => m.id !== monsterId).length > 0 && (
        <div className="mt-5">
